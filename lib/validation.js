...
function compileSchemasForValidation (context, compile) {
  ...
  if (headers && Object.getPrototypeOf(headers) !== Object.prototype) {
    ...
    context[headersSchema] = compile({ schema: headersSchemaLowerCase, originalSchema: context.originalSchema.headers, method, url, httpPart: 'headers' })
  }

  if (context.schema.body) {
    context[bodySchema] = compile({ schema: context.schema.body, originalSchema: context.originalSchema.body, method, url, httpPart: 'body' })
  }

  if (context.schema.querystring) {
    context[querystringSchema] = compile({ schema: context.schema.querystring, originalSchema: context.originalSchema.querystring, method, url, httpPart: 'querystring' })
  }

  if (context.schema.params) {
    context[paramsSchema] = compile({ schema: context.schema.params, originalSchema: context.originalSchema.params, method, url, httpPart: 'params' })
  }
}
